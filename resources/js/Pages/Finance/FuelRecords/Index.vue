...
export default {
    ...
    data() {
        return {
            ...
            driverId:     this.filters?.driver_id ?? '',
    ...
    methods: {
        applyFilters() {
            router.get('/fuel-records', {
                ...
                'filter[driver_id]':  this.driverId || undefined,
        ...
        executeDelete() {
            this.deleteForm.delete(`/fuel-records/${this.deleteTarget.id}`, {
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h1 class="text-xl font-semibold text-gray-900">Abastecimentos</h1>
                <Link href="/fuel-records/create" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-500 transition-colors">
                    + Novo Abastecimento
                </Link>
            </div>
        </template>
        ...
        <div class="mb-5 flex flex-wrap gap-3">
            ...
            <select v-model="driverId" @change="applyFilters"
                class="rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
        ...
        <div class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
            <table class="min-w-full divide-y divide-gray-100">
                <thead>
                    <tr class="bg-gray-50">
                        ...
                        <th class="px-6 py-3.5 text-right text-xs font-semibold uppercase tracking-wide text-gray-500">Litros</th>
                        <th class="px-6 py-3.5 text-right text-xs font-semibold uppercase tracking-wide text-gray-500">Preço/L</th>
                        <th class="px-6 py-3.5 text-right text-xs font-semibold uppercase tracking-wide text-gray-500">Total</th>
                        <th class="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Posto</th>
                        <th class="px-6 py-3.5" />
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 bg-white">
                    <tr v-if="!fuelRecords.data.length">
                        <td colspan="8" class="px-6 py-10 text-center text-sm text-gray-400">Nenhum abastecimento encontrado.</td>
                    </tr>
                    <tr v-for="r in fuelRecords.data" :key="r.id" class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 text-sm text-gray-700">{{ formatDate(r.fueled_at) }}</td>
                        <td class="px-6 py-4 text-sm text-gray-700">{{ r.vehicle?.license_plate ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-700">{{ r.driver?.name ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-right text-gray-900">{{ formatNumber(r.liters, 3) }}</td>
                        <td class="px-6 py-4 text-sm text-right text-gray-700">{{ formatCurrency(r.price_per_liter) }}</td>
                        <td class="px-6 py-4 text-sm text-right font-medium text-gray-900">{{ formatCurrency(r.total_cost) }}</td>
                        <td class="px-6 py-4 text-sm text-gray-500">{{ r.station ?? '—' }}</td>
                        <td class="px-6 py-4 text-right space-x-3">
                            <Link :href="`/fuel-records/${r.id}/edit`" class="text-sm font-medium text-indigo-600 hover:text-indigo-500">Editar</Link>
            ...
            <div v-if="fuelRecords.last_page > 1" class="border-t border-gray-100 px-6 py-4 flex items-center justify-between text-sm text-gray-500">
                <span>{{ fuelRecords.total }} registros</span>
                <div class="flex gap-2">
                    <Link v-if="fuelRecords.prev_page_url" :href="fuelRecords.prev_page_url" class="rounded px-3 py-1 hover:bg-gray-100">Anterior</Link>
                    <Link v-if="fuelRecords.next_page_url" :href="fuelRecords.next_page_url" class="rounded px-3 py-1 hover:bg-gray-100">Próximo</Link>
                </div>
            </div>
        </div>
        ...
        <div v-if="deleteTarget" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div class="w-full max-w-sm rounded-xl bg-white shadow-xl ring-1 ring-gray-200 p-6">
                <h3 class="text-base font-semibold text-gray-900">Excluir abastecimento?</h3>
